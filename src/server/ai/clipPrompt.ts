  ...
  "clips": [
    {
      ...
      "hashtags": ["#Faith"],
      "captionPackage": {
        "primaryCaption": "One to three grounded sentences that explain why the moment matters.",
        "shortCaption": "A concise feed caption under 140 characters.",
        "platformCaption": "A natural conversational alternative.",
        "optionalHashtags": ["#Faith"],
        "titleOptions": ["Clear grounded title", "Second distinct title"],
        "hookOptions": ["Grounded opening option", "Second distinct opening"],
        "ctaOptions": ["A ministry-appropriate next step"],
        "captionQualityScore": 8,
        "captionReason": "The copy reflects the spoken landing without exaggeration.",
        "captionWarnings": []
      },
      "score": 8,
      "reasonSelected": "",
...
export function buildClipSelectionSystemPrompt(): string {
  return [
    ...
    "Every title must read as a complete, specific promise or ministry idea on its own. Never return a transcript fragment, generic label, or unexplained pronoun as a title.",
    "Hooks should usually be 5-12 words, be understandable without prior sermon context, and stay faithful to words or ideas spoken in the selected range.",
    "The primary caption should use one to three natural sentences. The short caption should stay under 140 characters. Do not paste the full transcript as post copy.",
    "Write captions directly to the viewer. Never use detached meta narration such as 'the speaker says', 'the pastor explains', 'this clip shows', or 'in this sermon moment'.",
    "Use primaryCaption for the grounded ministry substance, shortCaption for one concise takeaway, and platformCaption for either a natural response question or a genuinely distinct conversational angle. Do not make these three fields paraphrases of the same sentence.",
    "Return four to eight focused optionalHashtags ordered from the exact sermon topic or Scripture, to the ministry category, then an optional church or series brand. Exclude reach-bait tags such as #fyp, #viral, #trending, and #explorepage.",
    "Hashtags must be single valid tokens, relevant to the selected transcript, and useful on a real church post. Do not pad the list with generic tags when the sermon evidence does not support them.",
    "CTA options must fit the moment: reflection, prayer, discussion, invitation, or sharing may be appropriate, but never add false urgency, guaranteed outcomes, or a generic encouragement CTA to every clip.",
    "Make titleOptions, hookOptions, captions, and CTA options meaningfully different from one another. Do not repeat the same sentence across fields.",
    "Use only these clipType values: inspirational, teaching, evangelistic, testimony, leadership, funny, prophetic, pastoral, worship.",
    ...
    clips: [
      {
        ...
        hashtags: ["#Faith"],
        captionPackage: {
          primaryCaption: "Faith grows when we choose the next faithful step before every answer is visible.",
          shortCaption: "Choose the next faithful step.",
          platformCaption: "What faithful step can you take today?",
          optionalHashtags: ["#Faith", "#SermonClip"],
          titleOptions: ["Choose the Faithful Step", "Faith Before Every Answer"],
          hookOptions: ["Faith moves before certainty", "Take the next faithful step"],
          ctaOptions: ["What step is God inviting you to take?"],
          captionQualityScore: 8,
          captionReason: "Each option is grounded in the selected teaching and landing.",
          captionWarnings: [],
        },
        score: 8,
...
export function buildClipRepairPrompt(rawResponse: string, validationError: string, windows: ClipWindow[] = []): string {
  ...
  return [
    ...
    windowDetails,
    "Allowed clipType values: inspirational, teaching, evangelistic, testimony, leadership, funny, prophetic, pastoral, worship.",
