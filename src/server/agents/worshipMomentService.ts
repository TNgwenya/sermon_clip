...
function candidateFromGroup(
  ...
  const transcriptText = selected.map((segment) => segment.text.trim()).join(" ");
  if (isSpokenProse(normalizeText(transcriptText))) {
    return null;
  }
