    ...
    <main className="growth-page-shell stack-lg">
      <header className="growth-hero">
        <div className="stack-sm">
          ...
          <p className="muted">
            Set up the platform connections that power growth recommendations, forecasts, and ministry impact reporting.
          </p>
        </div>
      ...
      <SectionCard title="OAuth connections" description="Authorize each platform once. Tokens are stored encrypted and used only for analytics sync until human-approved publishing is added.">
        <div className="growth-connector-list">
          <article className="growth-connector-row">
            <div>
              ...
              <p className="muted small">Scopes: YouTube Analytics readonly and YouTube readonly.</p>
            </div>
            {youtubeAuthHref ? <a href={youtubeAuthHref} className="button primary">Authorize</a> : <StatusBadge tone="warning">Missing env</StatusBadge>}
          </article>
          <article className="growth-connector-row">
            <div>
              <strong>Instagram / Facebook</strong>
              <p className="muted small">Redirect URI: <code>{buildOAuthRedirectUri("meta")}</code></p>
              <p className="muted small">Uses Meta Pages and Instagram business insights permissions.</p>
            </div>
            {metaAuthHref ? <a href={metaAuthHref} className="button primary">Authorize</a> : <StatusBadge tone="warning">Missing env</StatusBadge>}
          </article>
          <article className="growth-connector-row">
            <div>
              <strong>TikTok</strong>
              <p className="muted small">Redirect URI: <code>{buildOAuthRedirectUri("tiktok")}</code></p>
              <p className="muted small">Uses basic profile and video list analytics scopes.</p>
            </div>
            {tiktokAuthHref ? <a href={tiktokAuthHref} className="button primary">Authorize</a> : <StatusBadge tone="warning">Missing env</StatusBadge>}
          </article>
          <article className="growth-connector-row">
            <div>
            ...
            {threadsAuthHref ? <a href={threadsAuthHref} className="button primary">Authorize</a> : <StatusBadge tone="warning">Missing env</StatusBadge>}
          </article>
        </div>
      </SectionCard>

      <SectionCard title="Connector readiness" description="A connector is ready when its app credentials and at least one OAuth credential are present. Some APIs still require platform app review before insights return data.">
        <div className="growth-connector-list">
